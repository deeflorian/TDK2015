\chapter{Complex event processing}
	...
	\section{Complex Event Processing with the current technologies}
		In hierarchical runtime verification, the top level of modelling can be done by Complex Event Processing.
		...
		i.e. the time between the first allocation and the first release must be less than 10 seconds.\footnote{Ennel valami jobb kellene :S}
	...
	\section{Regular Expression}
		...
		$(a \rightarrow f)$ which means that an infinite sequences of allocation and release are allowed after each other. This is equivalent to the regular expression :		
		$(af)*$.
