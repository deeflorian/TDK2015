\chapter{System level runtime verification}
	...
	\section{Complex event processing using technologies}
		...
		This chapter going to introduce every step through an example, which is about the runtime verification of a two phase lock algorithm. %% TODO Reference
	...
	\section{Formal intermediate language}
		...
		\subsection{Timed regular expression}
			...
			\subsubsection{Timed eent automaton}
			...
			\subsubsection{Timed region automaton}
				...
				\begin{enumerate}
					...
					We have two cases, the chosen transition is:
						\begin{enumerate}
							\item Discrete transition: un case of $r^- = \emptyset$ than the execution of the transition is as in described formerly. 
				...
				Using the Timed Region Automaton we can compile our timed regular expression shown on \cref{fig:cep:trea}
		...
		\subsection{Parametric Timed Regular Expression}
			...
			$(a(\mathit{task},\mathit{\_resource})\ast r(\mathit{task},\mathit{\_resource})\ast a(\mathit{task},\mathit{\_resource}))| $
	...
	\section{Implementation}
		\subsection{Metamodel}
			...
			\subsubsection{Finite Automaton}
				...
				Every State has a flag to show if they are acceptor states.
			\subsubsection{Timing}
				The timing is done by Timed Regions. For each $\delta_t$ there is
				an instance of this class, with the timeout value, the included states and 
				the state which the token moves on timeout.

			\subsubsection{Parameters and Bindings}
				Parameters are implemented with the Parameter class hierarchy,
				for each token there is a set for every parameters, all of
				the parameters start as free parameters, and in case of binding, 
				the tokens split up to two tokens, one with the newly binded
				FixParameter on the new state, and the old tokens binded value
				is excluded from the possible values.
				
				The token -- event binding is done with the Binding class, which stands for the implementation on this.
		...
		\subsection{Executor}
			...
			If a token enters an acceptor state it'll call a previously defined method.
