...
The state charts definitions from \cref{sec:case_study:mon_sc} generates a VEPL pattern on the system monitor component, therefore we can write patterns, and and define actions for the patterns conclusions.
...
We can add rules to this stub, defining patterns for the error states in \cref{lst:case_study:gen_vepl}. 
...
With \cref{lst:case_study:ext_gen_vepl}, we use a very simple pattern to just match the generated events, and handle them. The \verb+ErrorHandler+ class is needed because from the rule we cannot access the model, therefore we need a static class which can modify the state of the model. For example, if this method sets the \verb+safe+ attribute of the \verb+Section+ with the id we informed, the following pattern can match:
