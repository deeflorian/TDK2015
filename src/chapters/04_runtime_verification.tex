...
\\\verb!statechart exStc(...) {...}!\\
where \verb!(...)! could be a parameter list, and the \verb!{...}! part contains the description of the statechart. The braces are optional and are only needed for parameters of statechart templates. Parametrized statecharts can be created from existing templates by providing a value for each parameter. If no parameters were specified, the definition is also treated as an instantiation of the statechart. Specifications must contain at least one instantiated statechart.
  \subsection{Regions}
...
\\\verb!assign !\textit{var}\verb! := !\textit{expression}\\
where \textit{var} is a global or local variable reference, and \textit{expression} is a valid expression.
...
Formal verification methods require a flatter model that the statechart language describes above. To enable formal verification of the defined systems, the language can be mapped to a transition system. This means that the complex concepts of the model are flattened out to easily verifiable ones. The transition system used has the following capabilities:
\begin{itemize}
